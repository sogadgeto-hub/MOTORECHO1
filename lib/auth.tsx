...
type AuthContextType = {
  ...
  updateAllowAiTraining: (allow: boolean) => Promise<{ error: string | null }>;
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function updateAllowAiTraining(allow: boolean): Promise<{ error: string | null }> {
    if (!user) return { error: 'Not authenticated' };

    try {
      const { error } = await supabase
        .from('profiles')
        .update({ allow_ai_training: allow, updated_at: new Date().toISOString() })
        .eq('id', user.id);
